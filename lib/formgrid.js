...
function displayable$1(source, settings) {
    return source && (source.hasOwnProperty(settings.field) && source[settings.field] != null);
}
...
function create_main_fragment$16(component, state) {
	...
	function select_block_type(state) {
		...
		if (state.displayable) { return create_if_block_1$2; }
		return null;
	}
...
function DataCol(options) {
	...
	this._recompute({ source: 1, settings: 1 }, this._state);
...
DataCol.prototype._recompute = function _recompute(changed, state) {
	if (changed.source || changed.settings) {
		if (this._differs(state.displayable, (state.displayable = displayable$1(state.source, state.settings)))) { changed.displayable = true; }
	}
