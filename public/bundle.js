...
function create_main_fragment$1(state, component) {
	...
	for (var i = 0; i < rows.length; i += 1) {
		each_blocks[i] = create_each_block(state, rows, rows[i], i, component);
	}
	...
	return {
		...
		p: function update(changed, state) {
			...
			if (changed.rows || changed.item || changed.edit || changed.editable) {
				for (var i = 0; i < rows.length; i += 1) {
					if (each_blocks[i]) {
						each_blocks[i].p(changed, state, rows, rows[i], i);
					} else {
						each_blocks[i] = create_each_block(state, rows, rows[i], i, component);
						each_blocks[i].c();
				...
				each_blocks.length = rows.length;
			}
		},
...
function create_each_block(state, rows, row, row_index, component) {
	...
	for (var i = 0; i < columns.length; i += 1) {
		each_blocks[i] = create_each_block_1(state, rows, row, row_index, columns, columns[i], i, component);
	}
	...
	return {
		...
		p: function update(changed, state, rows, row, row_index) {
			var columns = row.columns;

			if (changed.rows || changed.item || changed.edit || changed.editable) {
				for (var i = 0; i < columns.length; i += 1) {
					if (each_blocks[i]) {
						each_blocks[i].p(changed, state, rows, row, row_index, columns, columns[i], i);
					} else {
						each_blocks[i] = create_each_block_1(state, rows, row, row_index, columns, columns[i], i, component);
						each_blocks[i].c();
						each_blocks[i].m(each_anchor.parentNode, each_anchor);
...
function create_each_block_1(state, rows, row, row_index, columns, col, col_index, component) {
	var if_block_anchor;

	var current_block_type = select_block_type(state, rows, row, row_index, columns, col, col_index);
	var if_block = current_block_type(state, rows, row, row_index, columns, col, col_index, component);
	...
	return {
		...
		p: function update(changed, state, rows, row, row_index, columns, col, col_index) {
			if (current_block_type === (current_block_type = select_block_type(state, rows, row, row_index, columns, col, col_index)) && if_block) {
				if_block.p(changed, state, rows, row, row_index, columns, col, col_index);
			} else {
				if_block.u();
				if_block.d();
				if_block = current_block_type(state, rows, row, row_index, columns, col, col_index, component);
				if_block.c();
...
// (4:8) {{#if col.subtitle}}
function create_if_block(state, rows, row, row_index, columns, col, col_index, component) {
...
// (6:8) {{else}}
function create_if_block_1(state, rows, row, row_index, columns, col, col_index, component) {
	...
	return {
		...
		p: function update(changed, state, rows, row, row_index, columns, col, col_index) {
			...
			if (changed.editable) formcol_changes.editable = state.editable;
...
function select_block_type(state, rows, row, row_index, columns, col, col_index) {
...
function FormGrid(options) {
	...
	if (!('editable' in this._state)) console.warn("<FormGrid> was created without expected data property 'editable'");
